...
def get_model(hp, show_summary=True):
    ...
    units_3 = hp.Choice("units_3", values=[8, 16, 32])

    deep = tf.keras.layers.Dense(units_1, activation="relu")(concatenate)
    deep = tf.keras.layers.Dense(units_2, activation="relu")(deep)
    deep = tf.keras.layers.Dense(units_3, activation="relu")(deep)
...
def run_fn(fn_args):
    ...
    tuner = RandomSearch(
        ...
        hyperparameters=hp,
        directory=os.path.join(os.path.dirname(fn_args.serving_model_dir), "tuner"),
        project_name="stroke_pred",
    )
